...
const [deleteId, setDeleteId] = useState<string | null>(null);
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      showBanner("error", err instanceof Error ? err.message : "Server error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const uploadImages = async (files: File[]) => {
    ...
    for (const f of pick) {
      ...
      fd.append("file", f);
      fd.append("upload_preset", "unsigned_dashboard");
      ...
      if (!res.ok || !json.secure_url) throw new Error("Image upload failed");
  ...
  try {
    const res = await fetch("/api/admin/products", {
      ...
      body: JSON.stringify({
        ...
        price: payload.price,
        status: payload.status === "rejected" ? "pending" : payload.status, // 👈 هنا
    ...
    if (data.product) {
      ...
      closeEdit();
      showBanner("success", "تم تعديل المنتج ورجع لمرحلة المراجعة");
    }
  } catch (err) {
...
{/* Header */}
        ...
        {/* main table (same design as users) */}
        <div className="bg-white rounded-xl p-4 shadow-lg">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder={t.searchProductsPlaceholder} className="px-3 py-2 rounded bg-gray-50 border" />
          ...
          {loading ? (
            ...
            <div className="py-12 text-center text-slate-400">{t.noProducts}</div>
          ) : (
            <>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-700 hidden sm:table-cell">{t.descriptionproducts}</th>
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">{t.price}</th>
                      <th className="px-4 py-3 text-left text-sm font-medium text-gray-700 hidden md:table-cell">{t.category}</th>
                        ...
                        <td className="px-4 py-3">
                          <div className="w-16 h-12 rounded overflow-hidden bg-gray-100 flex items-center justify-center">
                            {p.images && p.images.length > 0 ? (
                              ...
                              <div className="text-xs text-slate-400 px-2">{t.noImage}</div>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="text-sm font-semibold text-gray-800">{p.name}</div>
                          <div className="text-xs text-slate-400 hidden sm:block">{new Date(p.createdAt || "").toLocaleDateString(lang === "ar" ? "ar-EG" : "en-US")}</div>
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-700 hidden sm:table-cell">{p.description}</td>
                        <td className="px-4 py-3 text-sm text-gray-700">${Number(p.price).toFixed(2)}</td>
                        <td className="px-4 py-3 text-sm text-gray-700 hidden md:table-cell">{p.category}</td>
...
<button
  ...
  disabled={busyAction}
  className={
    "inline-flex items-center gap-2 px-3 py-1 rounded bg-red-100 text-red-800 hover:scale-105 transition" +
    (busyAction ? " opacity-60 cursor-not-allowed" : "")
  }
>
  {t.delete}
</button>
  ...
  <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div className="bg-white rounded-lg p-6 shadow-lg max-w-sm w-full">
      <h3 className="text-lg font-semibold mb-3 text-gray-800">
        {t.confirmDeleteProduct ?? "هل أنت متأكد من حذف المنتج؟"}
      </h3>
        ...
        <button
          onClick={async () => {
            ...
            try {
              ...
              showBanner("success", t.productDeleted ?? "تم حذف المنتج");
            } catch (err) {
              const message = err instanceof Error ? err.message : "Delete failed";
              showBanner("error", message);
            } finally {
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg max-w-2xl w-full p-6 shadow-lg animate-fadeIn">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div>
                  <label className="text-sm text-gray-600">{t.descriptionproducts }</label>
                  <textarea className="w-full border rounded px-3 py-2 my-2" value={editing.description} onChange={(e) => setEditing({ ...editing, description: e.target.value })} />
                </div>

                <div className="md:flex gap-3">
                  <div className="flex-1">
                    <label className="text-sm text-gray-600">{t.price}</label>
                    <input type="number" className="w-full border rounded px-3 py-2 my-2" value={String(editing.price)} onChange={(e) => setEditing({ ...editing, price: Number(e.target.value) })} />
                  </div>
                  <div className="flex-1">
                    <label className="text-sm text-gray-600">{t.category}</label>
                    <input className="w-full border rounded px-3 py-2 my-2" value={editing.category} onChange={(e) => setEditing({ ...editing, category: e.target.value })} />
                  </div>
                </div>

                <div>
                  <label className="text-sm text-gray-600">صور (حتى 5)</label>
                  <div className="grid grid-cols-5 gap-2 my-2">
                    {editing.images && editing.images.slice(0,5).map((u, idx) => (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img key={idx} src={u} alt={`img-${idx}`} className="w-full h-20 object-cover rounded" />
                    ))}
                  </div>

                  <label className="flex flex-col items-center justify-center h-28 rounded-2xl border-2 border-dashed cursor-pointer mt-2">
                    <span>{imageFiles.length > 0 ? `${imageFiles.length} ${t.imagesSelected}` : t.selectImages}</span>
                    <input type="file" className="hidden" multiple accept="image/*" onChange={(e) => setImageFiles(Array.from(e.target.files || []).slice(0,5))} />
                  </label>
                </div>
              </div>
